        ...
        if elastic:
            ...
            if not plastic:
                ...
                while abs(dDfM) > 1e-9:
                    ...
                    Bij = (a1*self.I3 + a2*3./2*s2_dev/s2_eq +
                ...
                st_dev_tilde = st_dev - self.alpha()
                ...
                sy = None
                while residual > 1e-9:
                    sy = sy0 + (self._R + b*Q*DL)/(1 + b*DL)
                    dsydDL = b*Q/(1 + b*DL) - b*(b*Q*DL + self._R)/(1 + b*DL)**2

                    RA = R1 + R2*sy/sy0_au
                    dRAdDL = R2*dsydDL/sy0_au

                    Aij = sy*(st_dev - np.sum(self._alpha_m/(1 + np.outer(gamma*DL, np.ones(6))), 0))
                    dAijdDL = (dsydDL*(st_dev - np.sum(self._alpha_m/(1 + np.outer(gamma*DL, np.ones(6))), 0))
                               + sy*np.sum(self._alpha_m*np.outer(gamma, np.ones(6))
                                           / (1 + np.outer(gamma*DL, np.ones(6))), 0))

                    B = sy + 3*G*(DL + RA*DfM) + DL*np.sum(C*(1 + gamma*DL))
                    dBdDL = (dsydDL + 3*G*(1+dRAdDL) + np.sum(C*(1 + gamma*DL)*(1-gamma*DL/(1 + gamma*DL))))
                    sij_tilde = Aij/B

                    dsijdDL = (dAijdDL*B - dBdDL*Aij)/B**2
                    J11 = 3./2*matrix_double_contract(sij_tilde, dsijdDL)/sy - dsydDL
                    f = np.sqrt(3./2*matrix_double_contract(sij_tilde, sij_tilde)) - sy
                ...
                for i in range(self._alpha_m.shape[0]):
                    self._alpha_m[i, :] = (self._alpha_m[i, :] + C[i]*sij_tilde/sy*DL)*(1./(1+gamma[i]*DL))

                s2 = sij_tilde + self.alpha()
                s2[0:3] += (np.sum(st[0:3]) - K*dV*DfM)/3
                self._R = (self._R + b*Q*DL)/(1+b*DL)
            self._stress = s2
        self._strain = strain
